...
-- takes  Transn = ((State (int), Symbol (char)), State(int)) ,
-- (state (int), input[symbol = char])  and gives a state and input
-- if no state, head input pair exist -> deadstate, remaining input
-- TODO:
-- - base case
-- - inductive case
step trans config =
